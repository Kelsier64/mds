...
透過閱讀與實作，我**學到了更優雅的物件導向抽象化方式**（例如將狀態無關的外部工具如 `ls` 歸類在 `tools.py`，而將操作自身記憶的內部工具封裝在 `agent` 類別中），更**深刻體會到「關注點分離 (Separation of Concerns)」不只適用於程式碼模組化，也適用於 Agent 的職責劃分**（如 Magi-01 與 LTM-Manager）。

其實，以這項專案原本設定的其中一個目標「**實驗**」來說，結果並不算是成功。過於龐大的架構讓這套系統變得有點太複雜，導致 AI 的行為越來越不可控。但在此過程中，我慢慢發現**開發 AI 應用很重要的一點，就是要讓 AI 的互動環境越單純越好**（最近在ai應用圈流行起來的一個詞：「Harness」，正是這個概念）。

當初做這套系統，原本還有另一個目的是想幫自己做一個專屬的 Coding Agent：自己寫提示詞、自己處理工具調用，還有最重要的「記憶管理」。但實際執行後發現真的沒那麼簡單，這套系統寫完後的性能仍然輸給市面上主流的 Coding Agent 一大截，而且複雜的長期記憶問題仍然沒有被徹底解決。

為了解決這個痛點，我打算之後去實驗**「文檔驅動開發」**：利用 Agent Skill 讓 Coding Agent 自動化管理專案內的文檔，並以此來維持僅限於該專案的長期記憶。

雖然結果並不完美，但這個 Magi 專案還是讓我學到了非常多東西：我不僅實作出了一個挺好用的 ReAct 模式，也讓我直接學習並剖析了 Coding Agent 的底層架構。總得來說，這依然是一次非常充實的學習。
